...
const model=buildGaViewModel({project,breakers,boardSections,busRules,evaluations,dimensions:{heightMm:null,depthMm:null,heightStatus:'NOT_DEFINED',depthStatus:'NOT_DEFINED',label:'H Not Defined / D Not Defined'}});
assert.equal(model.boards.length,1);
assert.equal(model.boards[0].combinedPlanningWidthMm,2600);
assert.equal(model.boards[0].containsProvisionalWidth,true);
...
assert.equal(model.boards[0].dimensions.heightStatus,'NOT_DEFINED');
