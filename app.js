import { evaluateBreaker as sharedEvaluateBreaker, deriveBreakerType, framesForSeries, getAvailableManufacturerDimensions, getBusbarRule, getDesignConfigurationSchema, getProductCatalog, getRuleAudit } from './manufacturer-rules.js';
...
function renderSummary(status){
  ...
  status.boards.forEach(board=>{const sum=board.sections.reduce((acc,section)=>acc+section.width,0);const provisional=board.sections.filter(section=>section.widthStatus!=='VERIFIED').reduce((acc,section)=>acc+section.width,0);const line=document.createElement('span');line.textContent=(project.quantity===2?board.boardId+' ':'')+sum+' mm';total.append(line);const note=document.createElement('i');note.textContent=provisional?' incl. '+provisional+' mm Provisional Planning Width':' Verified Section Width';total.append(note)});
  ...
  $('#confidenceCounts').textContent='Devices Verified '+counts.manufacturerMatched+' · Confirmation Required '+counts.confirmationRequired+' · Provisional Sections '+counts.engineeringEstimate+' · Packing Unresolved '+counts.packingUnresolved+' · Invalid '+counts.invalid+' · Electrical Conflicts '+counts.electricalConflicts;
...
function applyTrace(title,b,e,bar,section){
  ...
  const deviceData={ruleId:e.ruleId,confidence:e.confidence,classification:e.classification,deviceWidth:e.widthMm+' mm'+(e.confidence==='Manufacturer Verified'?'':' (provisional / not matched)')};if(e.module)deviceData.module=e.module;if(e.operationalCurrent!=null)deviceData.operationalCurrent=e.operationalCurrent+' A';if(e.operationalCurrentStatus)deviceData.operationalCurrentStatus=e.operationalCurrentStatus;
  ...
  $('#spec').innerHTML='<b>'+esc(project.manufacturer+' '+project.system)+'</b>'+traceLines('INPUTS',inputs)+traceLines('DERIVED INPUTS',e.derivedInputs||{})+traceLines('USER CONFIGURATION',e.userConfiguration||{})+traceLines('MATCHED MANUFACTURER CONDITIONS',e.matchedConditions||[])+traceLines('DEVICE MANUFACTURER DATA',deviceData)+traceLines('SECTION PLANNING WIDTH',sectionData)+traceLines('MISSING CONDITIONS',[...(e.missingParameters||[]),...(section?.packingStatus==='UNRESOLVED'?section.packingMissingParameters.map(item=>'packing: '+item):[])])+traceLines('SWITCHBOARD DIMENSIONS',[dimension.label])+'<span>'+esc('Physical Busbar Position: '+bar.physicalPosition)+'</span>';
  $('#min').textContent=e.widthMm+' mm'+(e.module?' / '+e.module:'')+' · '+e.confidence;
  $('#rec').textContent=section?section.width+' mm · '+(section.widthStatus==='VERIFIED'?'Verified Section Width':'Provisional Planning Width · Engineering Estimate'):'-';
  $('#rule').textContent=e.ruleId;
  $('#why').textContent='DEVICE: '+e.widthMm+' mm · '+e.confidence+' · '+e.matchedRule+(section?' · SECTION: '+section.width+' mm '+section.widthLabel+' · PACKING: '+section.packingStatus:'')+' · Busbar: '+bar.manufacturerRule;
